...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-300'>
      <div
        onClick={() => navigate('/')}
        className='w-36 md:w-44 cursor-pointer font-bold text-2xl md:text-3xl text-primary select-none'
      ...
      <ul className='hidden md:flex items-center gap-5 font-medium'>
        ...
        {/* My Vitals button, only when logged in */}
        {token && (
          <NavLink to={'/user-dashboard'}>
            <li className='py-1 text-primary font-semibold'>MY VITALS</li>
            <hr className='border-none outline-none h-0.5 bg-primary w-full rounded-full m-auto hidden motion-preset-pop motion-duration-500' />
          </NavLink>
        )}
        ...
        <Link to={'/doctor-login'}>
          <button className='border border-primary text-primary hover:bg-primary/10 font-medium py-1.5 px-3 lg:py-2 lg:px-4 rounded-md transition-all duration-300 ease-in-out'>
            Doctor Login
          </button>
        </Link>
      </ul>
            ...
            <ul className='mt-10 uppercase flex flex-col-reverse items-center gap-7 text-base font-medium min-w-full select-none'>
              ...
              {token && (
                <NavLink onClick={() => setShowMenu(false)} to={'/user-dashboard'}>
                  <p className='text-primary font-semibold'>My Vitals</p>
                  <hr className='border-none outline-none h-0.5 bg-primary w-full rounded-full m-auto hidden' />
                </NavLink>
              )}
